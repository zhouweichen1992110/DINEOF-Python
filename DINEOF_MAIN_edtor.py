...
import numpy as np
from utils import select_set, init_missing, eof_multi, eof_core, normalize, denormalize,func_eofszb  # Import the EOF analysis function
...
# Perform filling using EOF estimation
dataFilled,_,_ = eof_core(dataInit, maskTest, best_eof_index, stopping, rounds)

# Denormalize the filled data (reverse the normalization process)
dataFilled = denormalize(dataFilled, 'std', normStds, 'mean', normMeans)

# Calculate test error (Mean Squared Error)
testError = np.mean((dataFilled.flatten()[testIndex] - testData) ** 2)
...
if np.any(np.isnan(data)):  # Check if there are any missing values in the dataset
    ...
    dataFilledFinal, _, _ = eof_core(dataInitFinal, maskFinal, best_eof_index, stopping, rounds)
...
for ss in range(0, 131, 25):  # 131 is the timestamps
    ...
    # save & plt
    plt.tight_layout()
    plt.savefig(f"./figs/DINEOF_Comparison_TimeStep_{ss+1}.png")  
    plt.show()  
    # plt.close(fig)  
